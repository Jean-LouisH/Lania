		...
		for (int j = 0; j < boxColliderCount; j++)
		{
			if (i != j)
			{
					...
					box2bottom > box1top))
				{
					...
					if (entity1->components.count(Entity2D::components::RIGID_BODY_2D) &&
						!entity2->components.count(Entity2D::components::RIGID_BODY_2D))
					{
						DynamicCollisionEvent2D dynamicCollisionEvent;
						RigidBody2D* rigidBody1 = &rigidBodies[entity1->components.at(Entity2D::components::RIGID_BODY_2D)];
						dynamicCollisionEvent.first.entityID = boxColliders[i].entityID;
						dynamicCollisionEvent.first.collider = Entity2D::components::BOX_COLLIDER_2D;
						...
						dynamicCollisionEvent.second.collider = Entity2D::components::BOX_COLLIDER_2D;
						...
						dynamicCollisionEvent.firstRigidBodyIndex = entity1->components.at(Entity2D::components::RIGID_BODY_2D);
						dynamicCollisionEvents->push_back(dynamicCollisionEvent);
					}
					else if (entity1->components.count(Entity2D::components::RIGID_BODY_2D) &&
						entity2->components.count(Entity2D::components::RIGID_BODY_2D))
					{
						DynamicCollisionEvent2D dynamicCollisionEvent;
						RigidBody2D* rigidBody1 = &rigidBodies[entity1->components.at(Entity2D::components::RIGID_BODY_2D)];
						RigidBody2D* rigidBody2 = &rigidBodies[entity2->components.at(Entity2D::components::RIGID_BODY_2D)];
						dynamicCollisionEvent.first.entityID = boxColliders[i].entityID;
						dynamicCollisionEvent.first.collider = Entity2D::components::BOX_COLLIDER_2D;
						dynamicCollisionEvent.first.elasticity_ratio = rigidBody1->elasticity_ratio;
						dynamicCollisionEvent.first.mass_kg = rigidBody1->mass_kg;
